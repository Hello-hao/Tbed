...
public class ClientController {
    ...
    public ResultBean clientupimg(HttpServletRequest request, @RequestParam("file") List<MultipartFile> file, @RequestParam(required = false) String apikey, @RequestParam(required = false) String email, @RequestParam(required = false) String pass) throws Exception {
        ...
        if (uploadConfig.getApi() == 1) {
            ...
            boolean iscon = false;
            if (apikey != null) {
                iscon = userService.isApiKeyExist(apikey);
                if (iscon) {
                    user = userService.getUsersByApiKey(apikey);
                }
            } else if (email != null && pass != null) {
                Integer ret = userService.login(email, Base64Encryption.encryptBASE64(pass.getBytes()), null);
                if (ret > 0) {
                    iscon = true;
                    user = userService.getUsers(email);
                }
            }
            if (iscon) {
                if (user.getIsok() == 1) {
                    User u = user;
//                        User u = userService.getUsersByApiKey(apikey);
                    ...
                    if (b) {
                        ...
                        resultBean = ResultBean.error(-1, "服务器内部错误，请联系管理员");
                    }
                }
            } else {
                resultBean = ResultBean.error(-2, "此用户信息不正确。");
            }
    } else

    {
        resultBean = ResultBean.error(-4, "管理员关闭了API接口");
    }
        return resultBean;
}
            ...
            if (email != null && pass != null) {
                ...
                if (ret > 0) {
                    ...
                    if (Sourcekey == 5) {
                        ...
                        bo = StringUtils.doNull(Sourcekey, key);//判断对象是否有空值
                    }
//        //容量判断
                    if (u == null) {
                        memory = uploadConfig.getVisitormemory();
                ...
                resultBean = ResultBean.error(-3, "邮箱或密码为空");
            }
        } else {
            resultBean = ResultBean.error(-4, "管理员关闭了API接口");
        }
